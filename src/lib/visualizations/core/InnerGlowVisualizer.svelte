<script lang="ts">
  ...
  const {
    ...
    colorTopRight = undefined,
    colorBottomLeft = undefined,
    colorBottomRight = undefined,
    sideVisible = 5,
    deformation = 10,
    blur = 12,
  }: Props = $props();
  ...
  $effect(() => {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      ctx.lineTo(
        amplitude * deformation + sideVisible,
        (i / values.length) * canvas.height,
      );
    }
    ctx.fill();
    ...
    for (let i = 0; i < values.length; i++) {
      const amplitude = values[i];
      ctx.lineTo(
        canvas.width * (1 - i / values.length),
        amplitude * deformation + sideVisible,
      );
    }
    ctx.fill();
